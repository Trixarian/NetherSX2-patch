...
def process_dict(my_dict, new_dict):
    req_sort = False
    my_dict = {k: v for k, v in my_dict.items() if any(rk in v for rk in key_list) or k in hwfkey_dict or k in gamefix_dict}
    for key, value in my_dict.items():
    ...
    for key, value in new_dict.items():
        ...
        if key in gamefix_dict and key in my_dict:
            for i in range(len(gamefix_dict[key])):
                if 'gameFixes' in my_dict[key]: my_dict[key]['gameFixes'].append(gamefix_dict[key][i])
